...
class CustomTrafficLightEnv(Env):
...
#group 1: have only 3 direction
        self.direction3 = np.zeros((3, 1))
#group 2: have 4 direction
        self.direction4 = np.zeros((4, 1))
        ...
        self.yellow_time = env_params.additional_params["yellow_time"]
    ...
    def get_state(self):
        ...
        state = [
            ...
            self.direction3.flatten().tolist(),
            self.direction4.flatten().tolist(),
    ...
    def _apply_rl_actions(self, rl_actions):
        ...
        for i, action in enumerate(rl_mask):
            ...
            if self.currently_yellow[i] == 1:  # currently yellow
                ...
                if self.last_change[i] == self.yellow_time:
                    #RG
                    i = j
                    if i > 3:
                        j=j-3 
                        if self.direction3[j] == 0:#3dir0
                            self.k.traffic_light.set_state(
                        ...
                        elif self.direction3[j] == 1: #3dir1
                            self.k.traffic_light.set_state(
                    ...
                    self.last_change[i] = 0.0 #enable to change for yellow
                    self.currently_yellow[i] = 0
            else: #yellow phase
                self.last_change[i] += self.sim_step
                if self.last_change[i] >= self.min_switch_time:
                    if action:
                        ...
                        if i > 3:
                            ...
                            if self.direction3[j] == 0:
                                self.direction3[j] = 1
                            elif self.direction3[j] == 1:
                                self.direction3[j] = 2
                            else:
                                self.direction3[j] = 0
                        ...
                        if i > 4:
                            ...
                            if self.direction4[j] == 0:
                                self.direction4[j] = 1
                            elif self.direction4[j] == 1:
                                self.direction4[j] = 2
                            elif self.direction4[j] == 2:
                                self.direction4[j] = 3
                            else:
                                self.direction4[j] = 0
    ...
    def _get_relative_node(self, agent_id, direction3, direction4):
        ID_IDX = 1
...
class CustomTrafficLightPOEnv(CustomTrafficLightEnv):
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        super().__init__(env_params, sim_params, network, simulator)
    ...
    def observation_space(self):
        tl_box = Box(
            low=0.,
            high=100000000000000000000000000000000000000000000000000,
...
class CustomTrafficLightBenchmarkEnv(CustomTrafficLightPOEnv):
    def compute_reward(self, rl_actions, **kwargs):
        """See class definition."""
        if self.env_params.evaluate:
            return - rewards.min_delay_unscaled(self)
...
class CustomTrafficLightTestEnv(CustomTrafficLightEnv):
    def _apply_rl_actions(self, rl_actions):
        """See class definition."""
        pass
