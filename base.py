...
class Network(object):
    ...
    * Network data can be collected from an sumo-specific network (.net.xml)
    ...
    def __init__(self,
        ...
        # this is to be used if file paths other than the the network geometry file is specified
            ...
            if 'rou' in net_params.template:
                ...
                # add the routes of the vehicles that will be departed later under the name of the vehicle. This will later be identified
    ...
    def specify_edge_starts(self):
        ...
        configuration file. Note that, the values are arbitrary but do not allow the positions of any two edges to overlap, thereby making them compatible with all starting position methods for vehicles.
    ...
    def specify_routes(self, net_params):
        ...
        * Alternatively, if the routes are meant to be stochastic, each element  can consist of a list of (route, probability) tuples, where the first element in the tuple is one of the routes a vehicle can take from a specific starting edge, and the second element is the probability that vehicles will choose that route. Note that, in this case, the sum of probability values for each dictionary key must sum up to one.
          ...
          For example, if we were to imagine the edge "right" in the ring road examples where split into two edges, "right_0" and "right_1", the routes for vehicles in this network in the probabilistic setting can
          be:
        ...
        * Finally, if you would like to assign a specific starting edge and route to a vehicle with a specific ID, you can do so by adding a element into the dictionary whose key is the name of the vehicle and whose content is the list of edges the vehicle is meant to traverse as soon as it is introduced to the network.
          ...
          As an example, assume we have 4 vehicles named 'human_0', 'human_1', 'human_2', and 'human_3' in the original ring road. Then, an
        ...
        dict
            ...
            Element = list of edges a vehicle starting from this edge must traverse *OR* a list of (route, probability) tuples for each
            starting edge
        """
        return None
    ...
    def _vehicle_infos(file_names):
        ...
        This is a utility function for computing vehicle information. It imports a network configuration file, and returns the information on
        ...
        for filename in file_names:
            ...
            # collect the departure properties and routes and vehicles whose properties are instantiated within the .rou.xml file. This will
            # only apply if such data is within the file (it is not implemented by networks in Flow).
            for vehicle in root.findall('vehicle'):
                # collect the edges the vehicle is meant to traverse
                route = vehicle.find('route')
                route_edges = route.attrib["edges"].split(' ')

                # collect the names of each vehicle type and number of vehicles of each type
            ...
            # collect the edges the vehicle is meant to traverse for the given sets of routes that are not associated with individual vehicles
    ...
    def _vehicle_type(filename):
        ...
        -------
        dict or None
            the key is the vehicle_type id and the value is a dict we've type of the vehicle, depart edges, depart Speed, departPos. If no
...
#additional
    ...
    def node_mapping(self):
        """Map nodes to edges.
        Returns a list of pairs (node, connected edges) of all inner nodes and for each of them, the 4 edges that leave this node. The nodes are listed in alphabetical order, and within that, edges are
